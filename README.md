...
- Sonnenschutz inklusive Nachführen nach Sonnenstand
...
## 2. Voraussetzungen
 ...
 - Die Statusvariablen sollten nicht emuliert werden. Dies ist wichtig, um manuell ausgelöste Bewegungen eindeutig von automatisch ausgeführten Bewegungen unterscheiden zu können. Meldet der Aktor die erreichte Position erst mit Verzögerung (z. B. KNX-Aktoren, die den Status erst nach Fahrtende senden), wird eine solche verspätete Rückmeldung der eigenen Fahrt aber erkannt und nicht als manuelle Bedienung gewertet: Entspricht die gemeldete Position der zuletzt vom Modul kommandierten (Toleranz: „minimale Bewegung"), bleibt die Automatik aktiv.
...
Um auf offene Fenster/Türen oder auch Regen/Sturm reagieren zu können, lassen sich bis zu vier Kontakte definieren. Im klassischen Ein/Aus-Modus wird ein Kontakt als offen erkannt, wenn der Wert >0 ist; bei Kontakten, die im geöffneten Zustand den Wert 0 aufweisen, ist ein 'Reversed'-Profil zu verwenden. Werden stattdessen Wertegruppen konfiguriert (siehe unten), zählt der Rohwert-Vergleich statt der >0-Erkennung.
...
Die **Kontakte zum Öffnen** unterstützen zusätzlich **Wertegruppen**: Statt einer reinen Ein-/Aus-Auswertung können je Kontakt bis zu drei Rohwert-Gruppen (kommasepariert, Typ Boolean/Integer/Float/String) mit jeweils eigener Position (Höhe und Lamellen) hinterlegt werden. Bei numerischen Variablen (Integer/Float) sind neben exakten Werten auch **Vergleiche und Bereiche** möglich: `>x`, `>=x`, `<x`, `<=x` sowie `a-b` (Grenzen inklusive) — z. B. `>0.2` oder `0.2-0.8`. Dabei immer den **Dezimalpunkt** verwenden, das Komma trennt die Werte. Die kommaseparierten Werte einer Gruppe sind **oder**-verknüpft — `>3,<5` trifft daher auf jeden Wert zu. Ein Bereich mit ausgeschlossenen Grenzen lässt sich über engere Grenzen abbilden (z. B. `3.001-4.999` für „zwischen 3 und 5 ohne die 3 und die 5"; bei Integer-Werten genügt `4`). Bleiben alle Wertefelder eines Kontakts leer, verhält er sich wie bisher als Ein/Aus-Kontakt mit der klassischen Höhe/Lamelle (`ContactOpenLevel{i}` / `ContactOpenSlatsLevel{i}`). Passt der aktuelle Wert zu keiner Gruppe, hat der Kontakt keine Wirkung. Die Gruppen werden in der Reihenfolge 1 bis 3 geprüft; die erste passende Gruppe bestimmt die Position. Je Öffnen-Kontakt kann eine eigene Verzögerung (Entprellung) gesetzt werden; jedes weitere Telegramm startet die Wartezeit neu, sodass nur der zuletzt gemeldete Wert ausgewertet wird. Die Entprellung ist für prellende Kontakte und Griffe gedacht: Bei Variablen, die laufend neue Werte liefern (z. B. ein Float-Windsensor), startet die Wartezeit mit jeder Wertänderung neu, und die Auswertung kann sich beliebig weit verschieben — dort die Verzögerung auf 0 lassen und den Messwert ggf. über eine Helfervariable beruhigen (siehe unten).

Ein Fenstergriff mit drei Positionen kann über diese Wertegruppen abgebildet werden: Die Positionsvariable des Griffs als Öffnen-Kontakt auswählen und die Rohwerte je Stellung als Wertegruppe hinterlegen – z.B. bei SODA `left,right` für offen und `up` für gekippt als zwei Positionen; `down` passt zu keiner Gruppe und hat damit keine Wirkung (geschlossen). Da ein Griff während der Drehbewegung mehrere Zwischenpositionen senden kann, empfiehlt sich eine Entprellung (z.B. 1 Sekunde) über die Verzögerung des jeweiligen Kontakts.

Ein Beispiel für die Vergleichssyntax: Soll der Rollladen unter einer Markise geöffnet werden, sobald die Markise ausgefahren ist, die LEVEL-Variable der Markise (Bereich 0.0 bis 1.0) als Öffnen-Kontakt auswählen und als Rohwert für Position 1 z. B. `>0.05` eintragen — jede nennenswerte Ausfahrposition fährt den Rollladen dann auf die eingestellte Mindesthöhe; bei eingefahrener Markise (Wert 0) greift wieder die normale Automatik.

Die Vergleiche kennen **keine Hysterese**: Die Bedingung wird bei jeder Auswertung exakt geprüft. Pendelt ein Messwert um den Grenzwert (z. B. Wind um `>20`), wechselt der Kontaktstatus entsprechend oft hin und her — auch die „Zeit der Deaktivierung nach automatischer Fahrt" verhindert das nicht, da sie bei Kontakten bewusst nicht greift. Für solche Messwerte empfiehlt sich eine kleine Helfervariable, die per Ereignis/Skript mit eigener Hysterese bzw. Haltezeit gesetzt wird (z. B. „Wind" ein ab 20, aus erst nach 15 Minuten unter 15); dem Modul wird dann nur der stabile Zustand als Kontakt zugewiesen.

Die **Kontakte zum Schließen** unterstützen dieselben Wertegruppen und dieselbe Entprellung wie die Kontakte zum Öffnen. Sind mehrere Schließen-Kontakte gleichzeitig offen, wird die am weitesten geschlossene Position angefahren, sodass die restriktivste Anforderung erfüllt wird.
...
| `TemperatureIDShadowingBySunPosition` | integer | 0            | Indikatorvariable die einen Temperatursensor (Außentemperatur) wiedergibt. Der Wert wirkt **zweifach** – auf den Helligkeitsschwellwert (Beschattungsbeginn) und auf die Behanghöhe (Hitzeschutz: über 27 °C weitere 15 % herunter, über 30 °C auf 90 % Schließungsgrad). Zum Helligkeitsschwellwert: Bei Temperaturen über 24 °C wird der Helligkeitsschwellwert verringert, um eine frühere Beschattung zu erreichen (10% Schwellwertverringerung je Grad Temperaturdifferenz zu 24 °C). Bei Temperaturen unter 10 °C wird der Helligkleitsschwellwert heraufgesetzt, um die Sonne erst später 'auszusperren' (10% Schwellwerterhöhung je Grad Temperaturdifferenz zu 10 °C).                                                                                 |
| ***Bestimmung des Beschattungsgrades - einfache Variante***<br>`LowSunPositionAltitude`<br>`HighSunPositionAltitude`<br>`LowSunPositionBlindLevel`<br>`HighSunPositionBlindLevel`<br>`LowSunPositionSlatsLevel`<br>`HighSunPositionSlatsLevel` |  float  | 0            | Aus diesen möglichst weit auseinanderliegenden Wertepaaren wird die Behanghöhe und die Lamellenposition in Abhängigkeit von der Sonnenhöhe errechnet                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `WindowsSlope` | integer | 90           | Neigung der Fensterfläche, **gemessen gegen die Horizontale**: senkrechtes Fassadenfenster = 90°, flach liegendes Dachfenster = 0°. Bei geneigten Dachfenstern entspricht der Wert der Dachneigung (Beispiel: Dach mit 45° Neigung → 45°).                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| <br>**Kontakte**                                                                                                                                                          |         |              |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | |
| `ContactOpen1ID`, `ContactOpen2ID` | integer | 0            | Indikatorvariablen (Boolean, Integer, Float oder String). Ohne Wertegruppen wie bisher als Ein/Aus-Kontakt; mit Wertegruppen wird je Rohwert eine der drei Positionen angefahren.                                                                                                                                                                                                                                                                                                                                                     |
| `ContactOpenLevel1`, `ContactOpenLevel2` |  float  | 0            | Mindesthöhe für **Position 1**. Ohne Wertegruppen ist dies zugleich die Ein/Aus-Öffnungshöhe (bestehendes Property, keine Migration nötig).                                                                                                                                                                                                                                                                                                                                                                                           |
| `ContactOpenSlatsLevel1`, `ContactOpenSlatsLevel2` |  float  | 0            | Minimale Lamellenposition für Position 1 bzw. den Ein/Aus-Modus.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `ContactOpenValues1_1`, `ContactOpenValues1_2`, `ContactOpenValues1_3`, `ContactOpenValues2_1`, `ContactOpenValues2_2`, `ContactOpenValues2_3` | string  | (leer)       | Optionale kommaseparierte Rohwerte je Kontakt i∈{1,2} und Position 1–3. Passt der aktuelle Wert zu einer Gruppe, wird die zugehörige Position angefahren. Ein Wert, der zu keiner Gruppe passt, hat keine Wirkung – es gilt wieder die normale Automatik. Den Normalzustand (z. B. Fenster geschlossen) daher keiner Gruppe zuordnen.                                                                                                                                                                                                                                                                                                                    |
| `ContactOpenLevel1_2`, `ContactOpenLevel2_2`, `ContactOpenLevel1_3`, `ContactOpenLevel2_3` |  float  | 0            | Mindesthöhe der Zusatzpositionen 2 und 3 je Kontakt (Position 1 = `ContactOpenLevel{i}`).                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `ContactOpenSlatsLevel1_2`, `ContactOpenSlatsLevel2_2`, `ContactOpenSlatsLevel1_3`, `ContactOpenSlatsLevel2_3` |  float  | 0            | Minimale Lamellenposition der Zusatzpositionen 2 und 3 je Kontakt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `ContactOpenDelay1`, `ContactOpenDelay2` | integer | 0            | Verzögerung in Sekunden je Öffnen-Kontakt, bevor der zuletzt gemeldete Wert ausgewertet wird. Jedes weitere Telegramm startet die Verzögerung neu. Mit `0` erfolgt die Auswertung sofort. **Achtung:** Bei laufend sendenden Messwert-Variablen (z. B. Wind) verschiebt jede Wertänderung die Auswertung erneut — hier die Verzögerung auf 0 lassen.                                                                                                                                                                                                                                                                                                                                             |
| `ContactClose1ID`, `ContactClose2ID` | integer | 0            | Indikatorvariablen (Boolean, Integer, Float oder String). Ohne Wertegruppen wie bisher als Ein/Aus-Kontakt; mit Wertegruppen wird je Rohwert eine der drei Positionen angefahren.                                                                                                                                                                                                                                                                                                                                                     |
| `ContactCloseLevel1`, `ContactCloseLevel2` |  float  | 0            | Maximalhöhe für **Position 1**. Ohne Wertegruppen ist dies zugleich die Ein/Aus-Schließhöhe (bestehendes Property, keine Migration nötig).                                                                                                                                                                                                                                                                                                                                                                                            |
| `ContactCloseSlatsLevel1`, `ContactCloseSlatsLevel2` |  float  | 0            | Maximale Lamellenposition für Position 1 bzw. den Ein/Aus-Modus.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `ContactCloseValues1_1`, `ContactCloseValues2_1`, `ContactCloseValues1_2`, `ContactCloseValues2_2`, `ContactCloseValues1_3`, `ContactCloseValues2_3` | string  |              | Kommaseparierte Rohwerte je Position (bis zu drei) eines Schließen-Kontakts. Bleiben alle leer, gilt das klassische Ein/Aus-Verhalten. Passt der Wert zu keiner Gruppe, hat der Kontakt keine Wirkung – es gilt wieder die normale Automatik. Den Normalzustand daher keiner Gruppe zuordnen.                                                                                                                                                                                                                                                                                                                                |
| `ContactCloseLevel1_2`, `ContactCloseLevel2_2`, `ContactCloseLevel1_3`, `ContactCloseLevel2_3` |  float  | 0            | Maximalhöhe der Zusatzpositionen 2 und 3 je Kontakt (Position 1 = `ContactCloseLevel{i}`).                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `ContactCloseSlatsLevel1_2`, `ContactCloseSlatsLevel2_2`, `ContactCloseSlatsLevel1_3`, `ContactCloseSlatsLevel2_3` |  float  | 0            | Maximale Lamellenposition der Zusatzpositionen 2 und 3 je Kontakt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `ContactCloseDelay1`, `ContactCloseDelay2` | integer | 0            | Verzögerung in Sekunden je Schließen-Kontakt, bevor der zuletzt gemeldete Wert ausgewertet wird. Jedes weitere Telegramm startet die Verzögerung neu. Mit `0` erfolgt die Auswertung sofort. **Achtung:** Bei laufend sendenden Messwert-Variablen (z. B. Wind) verschiebt jede Wertänderung die Auswertung erneut — hier die Verzögerung auf 0 lassen.                                                                                                                                                                                                                                                                                                                                          |
...
| `WriteLastDecision` | boolean | false        | legt fest, ob die optionale Statusvariable LAST_DECISION angelegt wird, die dokumentiert, warum sich der Rollladen beim letzten Steuerungslauf bewegt hat oder nicht (siehe Abschnitt Statusvariablen).                                                                                                                                                                                                                                                                                                                               |
| `WriteDecisionTrace` | boolean | false        | legt fest, ob die optionale Statusvariable DECISION_TRACE angelegt wird, die das vollständige Ablaufprotokoll des letzten Steuerungslaufs als HTML festhält (siehe Abschnitt Statusvariablen).                                                                                                                                                                                                                                                                                                                                        |
| `WriteLogInformationToIPSLogger` | boolean | false        | legt fest, ob die Log Informationen zusätzlich zum Standard Logfile auch an den IPSLogger der IPSLibrary übergeben werden sollen (setzt eine installierte IPSLibrary voraus).                                                                                                                                                                                                                                                                                                                                                        |
...
Sie dokumentiert die Entscheidung des jeweils letzten Steuerungslaufs und beantwortet damit die Frage, **warum sich der Rollladen bewegt hat – oder eben nicht**. Um wiederholt identische Einträge (z. B. dauerhaft „Zielposition bereits erreicht") zu vermeiden, wird die Variable nur dann aktualisiert, wenn sich die Entscheidung gegenüber dem vorherigen Lauf geändert hat.
...
Eine **verspätete Positions-Rückmeldung des Aktors** wird dabei nicht als manuelle Bedienung gewertet: Meldet die Statusvariable erst nach Fahrtende die Position der letzten Automatikfahrt (typisch bei KNX-Aktoren mit langer Fahrzeit), erkennt das Modul die Änderung als Rückmeldung der eigenen Fahrt, sofern die gemeldete Position der zuletzt kommandierten entspricht (Toleranz: „minimale Bewegung"). Im Trace erscheint dann z. B. `Bewegungssperre: keine (Positionsänderung um 09:35 entspricht der zuletzt kommandierten Position (Aktor-Rückmeldung, keine manuelle Bedienung))`.

In der Beschattungs-Zeile wird der für die Beschattungsentscheidung **tatsächlich verwendete** Helligkeitswert zusammen mit dem Schwellwert angezeigt. Dieser Wert kann von der in der Zeile „Tageszeit" genannten Helligkeit abweichen: Zum einen kann für die Beschattung ein anderer Helligkeitssensor konfiguriert sein als für die Tag/Nacht-Erkennung, zum anderen wird für die Beschattung – sofern eine Mittelung über mehrere Minuten eingestellt ist – ein effektiver Wert aus aktuellem und gemitteltem Messwert herangezogen. Wird tatsächlich der gemittelte Wert verwendet, ist das im Trace gekennzeichnet, z. B. `Helligkeit 45000 lx (Mittelwert über 10 min)` — auch in der Zeile „Tageszeit", wenn dort ein Durchschnittswert einfließt.
...
- Der Helligkeits-Schwellwert wird temperaturabhängig angepasst (10 % je Grad über 24 °C bzw. unter 10 °C). Greift diese Anpassung, wird der Schwellwert zusammen mit dem konfigurierten Ausgangswert ausgewiesen, z. B. `Schwellwert 21000 lx (temperaturkorrigiert von 70000 lx, 31 °C)` — so ist der eigene, eingestellte Schwellwert direkt wiederzuerkennen. Derselbe Vermerk erscheint auch in der Begründung, wenn wegen zu geringer Helligkeit nicht beschattet wird (`Helligkeit … unter Schwellwert … (temperaturkorrigiert von …)`).
